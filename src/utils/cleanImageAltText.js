...
export function cleanImageAltText(html) {
  ...
  imageFigures.forEach((figure) => {
    ...
    const hasAltPrefix = /^(Alt+|alt+|ALT+)\s*:/i.test(text);
  ...
  figcaptions.forEach((figcaption) => {
    ...
    html = html.replace(/(Alt+|alt+|ALT+)\s*:\s*[^<\n]*/gi, '');
  ...
  paragraphs.forEach((p) => {
    ...
    const isOnlyAlt = /^(Alt+|alt+|ALT+)\s*:/i.test(text);
  ...
  result = result.replace(
    /<!-- wp:paragraph[^>]*-->\s*<p[^>]*>(Alt+|alt+|ALT+)\s*:[^<]*<\/p>\s*<!-- \/wp:paragraph -->/gi,
    ''
  );
