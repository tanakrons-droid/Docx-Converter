...
export function convertReadMoreLinks(html) {
  ...
  paragraphs.forEach(paragraph => {
    ...
    // 🔒 ป้องกันการแปลงซ้ำ - ตรวจสอบ data-readmore attribute
    if (paragraph.hasAttribute && paragraph.hasAttribute('data-readmore')) {
      return; // ข้ามการแปลงถ้าถูกแปลงไปแล้ว
    }
    
    // 🔒 ป้องกันการแปลงซ้ำ - ตรวจสอบว่าอยู่ใน WordPress block แล้วหรือไม่
    if (paragraph.classList && paragraph.classList.contains('vsq-readmore')) {
      return; // ข้ามการแปลงถ้าเป็น readmore block อยู่แล้ว
    }
