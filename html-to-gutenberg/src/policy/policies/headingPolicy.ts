...
const defaultOptions: HeadingPolicyOptions = {
  minCount: 1,
  autoGenerate: false,
 ...
 */
export const headingPolicy: Policy = {
  name: 'requireH2',
  ...
  apply(html: string, $: CheerioAPI, options: PolicyOptions = {}): PolicyResult {
    const opts = { ...defaultOptions, ...options } as HeadingPolicyOptions;
    ...
    if (opts.autoGenerate) {
      ...
      return createWarningResult(
        $.html(),
        [`เพิ่มหัวข้อ H2 อัตโนมัติ ${missing} หัวข้อ (ต้องการ ${minCount}, มี ${currentCount})`],
        [`auto-generated ${missing} H2 heading(s)`]
      );
    }
    ...
    return createFailedResult(
      html,
      [`บทความต้องมีหัวข้อ H2 อย่างน้อย ${minCount} หัวข้อ (พบ ${currentCount} หัวข้อ)`]
    );
  }
};
