...
const defaultOptions: RemoveInternalNotesPolicyOptions = {
  autoRemove: true,
 ...
 */
function isInternalNote(text: string, patterns: string[]): boolean {
 ...
 */
export const removeInternalNotesPolicy: Policy = {
  ...
  apply(html: string, $: CheerioAPI, options: PolicyOptions = {}): PolicyResult {
    const opts = { ...defaultOptions, ...options } as RemoveInternalNotesPolicyOptions;
    const patterns = opts.patterns || [];

    const foundNotes: InternalNoteMatch[] = [];
    ...
    $('p, div, li, td').each((_: number, el: any) => {
      const element = $(el);
    ...
    if (opts.autoRemove) {
      ...
      return createWarningResult(
        $.html(),
        [message],
        [`removed ${removedTexts.length} internal note(s)`]
      );
    }
    ...
    return createFailedResult(
      html,
      [`พบข้อความภายใน ${removedTexts.length} รายการที่ต้องลบ: ${summary}`]
    );
  }
};
