...
const defaultOptions: RemoveBeforeH1PolicyOptions = {
  autoRemove: true
};
 ...
 */
export const removeBeforeH1Policy: Policy = {
  name: 'removeBeforeH1',
  ...
  apply(html: string, $: CheerioAPI, options: PolicyOptions = {}): PolicyResult {
    const opts = { ...defaultOptions, ...options } as RemoveBeforeH1PolicyOptions;
    ...
    return createWarningResult(
      $.html(),
      [`ลบเนื้อหาก่อน H1 และ H1 แรก จำนวน ${removedCount} รายการ (แท็ก: ${uniqueTags})`],
      [`removed ${removedCount} element(s) before and including first H1`]
    );
  }
};
