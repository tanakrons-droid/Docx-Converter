 ...
 */
export function extractStyles(html: string): StyleExtractionResult {
 ...
 */
export function extractAllStyles(html: string): ExtendedStyleExtractionResult {
  const $ = cheerio.load(html);
  const styleContents: string[] = [];
 ...
 */
function extractInlineStyles($: cheerio.CheerioAPI): Map<string, Record<string, string>> {
  const inlineStyles = new Map<string, Record<string, string>>();
  let elementIndex = 0;
 ...
 * @returns CSSClassMap
 */
export function parseCSS(css: string): CSSClassMap {
 ...
 */
export function parseCSSExtended(css: string): {
  cssMap: CSSClassMap;
  ...
  const selectorMap: SelectorStyleMap = {};
  const mediaQueries: Array<{ query: string; rules: SelectorStyleMap }> = [];
 ...
 */
function parseRulesFromCSS(css: string): SelectorStyleMap {
  const rules: SelectorStyleMap = {};
  const ruleRegex = /([^{]+)\{([^}]*)\}/g;
  let match;
 ...
 */
export function parseProperties(properties: string): Record<string, string> {
 ...
 */
function splitCSSDeclarations(css: string): string[] {
 ...
 */
export function propertiesToInlineStyle(properties: Record<string, string>): string {
